...
Write each assertion as one requirement a reader can mark pass or fail from the output alone. A grader turns it into a single boolean question, so an assertion with two clauses, or one the reader cannot label the same way twice, measures nothing; split or rewrite it. Graded pass rates mean something only after the grader agrees with your labels on a sample: [agent-evals](https://github.com/mblode/agent-evals) `judge` grades assertions with a cheap evaluation model, compares with-skill and without-skill runs, and reports that agreement.
