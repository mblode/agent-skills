...
**Clarity.** Plain words. Cut filler (very, really, just, basically, in order to), but keep "only" or "may" when it carries a limit or uncertainty. Specific with a measured number when you have one, otherwise cut the adjective; never invent a figure or a time-to-complete. Spelling follows the profile, then the page's established locale, then US English; code identifiers stay exact. One idea per sentence; a sentence over 25 words with two "and"s is a candidate to split, not a failure by length. Verbs, not nominalisations.
