...
- Excluding containers from a service stack saves the image pulls, not the boot: a Supabase start that lost seven containers went from 2:47 to 2:11, because Postgres init and the health waits on the remaining containers were most of it. Predict the saving from a log of the step, not from the container count.
- Vitest and Playwright shard by file count, so two shards of the same suite can differ by a minute of tests. Read both shards' durations before adding a third; splitting the long files rebalances, another shard does not.
- A background boot must resolve its project the same way the step that reads it does. `supabase start --workdir <dir>` does not walk up to the config the way a cwd lookup does; it booted a default project under another container name and the later `supabase status` found nothing. Run both from the same directory, and print the container names in the wait step.
- Overlapping two IO-bound steps on a hosted runner returns less than their sum. A Supabase boot run in the background beside `npm ci` cut the post-install wait from 2:11 to 0:34, but the install grew from 1:32 to 2:28 because pulls and package extraction share one network link and one disk. Predict the overlap from the runner's IO, not from the two durations.
