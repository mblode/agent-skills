...
10. **Over-memoisation**: remove `useCallback`/`useMemo` that adds no value: wrapping a `setState` updater (the updater form is already stable), memoising with Apollo query objects as deps (busts on every refetch), or computing a value used only in the same component. Keep `useCallback` only when the reference is passed to a child that would otherwise re-render, and `useMemo` only when the computation is demonstrably expensive. Under React Compiler (`reactCompiler: true` in Next.js, or the Babel plugin), new code needs neither, so flag manual memoisation that this diff adds; leave pre-existing memoisation alone, since React's own guidance is that removing it can change the compiler's output and it earns a test pass of its own
