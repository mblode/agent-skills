...
Answer these four questions in order before writing animation code. `defaults.md` carries the duration table, the named curves, and the pattern-to-recipe map; this file is the reasoning that picks between them.
...
Two cases the named curves in `defaults.md` do not cover:
