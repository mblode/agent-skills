...
Name the material candidates the audit looked at and deliberately did not flag, each with the guard that killed it. There is no quota: an empty list is honest when nothing plausible was set aside, and an invented rejection is padding.
...
Audit self-check: ✓ 14 rules run · ✓ counts reconcile (8 = 3 + 5)
                  ✓ every applied fix names its edit · ✓ check-report.mjs passed
═══════════════════════════════════════════════════════════
...
- `CONSIDERED AND REJECTED` lists material rejections only; omit the block when there are none.
- Close with the defer-to, suppression, and self-check footers.

A clean run is short and stays short (its rejections are the ones the pass genuinely weighed):
...
`audit.selfCheck.failures[]` is empty on a clean audit. `scripts/audit/check-report.mjs` checks the document mechanically (counts, verdict, required fields, rule IDs that exist, applied edits inside scope). Codes:
...
- Every `consideredAndRejected` entry has `candidate`, `rule`, and `guard`.
