...
The audit's value is the gap between "lint passes and axe is clean" and "the product still feels broken." It does not duplicate what other tools handle well. When a finding falls into another tool's territory, link out.
...
**Dispatch** goes to Verify mode, which is not another vendor's product but this skill's own browser session. It runs axe against the rendered page in both themes, measures hit areas and layout shift, walks focus, injects 500s, and hands back a measurement keyed to the rule id that raised the candidate. Where a running app exists, an audit dispatches rather than defers, and the finding arrives with `32x32px at 360px width` instead of `h-8 on line 42`. `references/verify/rule-coverage.md` maps every rule in this corpus to the probe that decides it.
...
|---|---|---|
| LCP, CLS, INP, FCP, TTFB measurement | **Lighthouse** + **web-vitals** library + **Vercel Agent** | Field + lab measurement; the audit reads source and inspects a rendered viewport, it does not measure field performance |
| WCAG 2.x rule violations | **axe-core** (runtime, dispatched to Verify mode) + **eslint-plugin-jsx-a11y** (lint) | Authoritative WCAG rule list with structured violations |
| `alt` text, `aria-*` attribute presence | **eslint-plugin-jsx-a11y** | Catches at write time |
| Color contrast ratios | **axe-core** per theme (dispatched to Verify mode) + **Storybook a11y addon** | Computed contrast per element, and it differs between light and dark |
...
## What the audit catches that none of the above catch

The high-leverage gaps; the audit's reason to exist:

| Gap | Tools that miss it | Rule |
|---|---|---|
...
**Also run:** `lighthouse --only-categories=performance` to confirm CLS budget; Lighthouse measures the metric, the audit catches the static cause.
```

The audit summary always lists deferred categories explicitly:

```text
Verified in the browser (Verify mode):
...
This sets expectations and prevents the "why didn't the audit catch X" complaint when X is another tool's job.
