...
Surface type defects are the `type-` rules; punctuation, OpenType, pairing, brand, and display type are the typography scope (`rules-typography/`). Not duplicated here.
...
Timing, easing, springs, gestures, and transform/opacity-only animation belong to Motion mode. Route motion findings there for the fix.
