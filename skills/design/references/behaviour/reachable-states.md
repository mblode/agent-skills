...
Load in Behaviour shape, spec, and harden. A happy-path mockup is incomplete, not done: design every state the surface can actually enter, and only those.
...
- Transitions: what changes on action, what animates (Motion mode owns the motion).
...
- Optimistic UI without a rollback path is a happy-path shortcut, not a complete state (Audit mode checks the built rollback under `async-optimistic-without-rollback`).
...
- Long content never pushes the confirm and cancel actions out of reach. Keeping the actions reachable is the behaviour decision; whether the modal body scrolls correctly is Audit mode's rendered check.
...
These states are decided here; whether the built UI renders them correctly is Audit mode's check (`layout-long-content-safety`, `dark-i18n-rtl-untested`).
