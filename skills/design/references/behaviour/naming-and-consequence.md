...
- Behaviour mode decides: whether the action should exist, which object it affects, its scope, its consequence, whether it is reversible, and which safeguard pattern that implies.
- `ghostwriter` writes: the exact strings, the canonical verb per operation, and error, success, empty, loading, and permission copy, against the copy rule IDs in `product-rules.md`.
