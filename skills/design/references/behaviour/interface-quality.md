...
Load in `review` and `harden` modes. Holds the accessibility-as-task-completion standard, the review scan order, and the severity rubric and finding format. Not visual aesthetics or implementation audits: both belong to other modes (Direction and Build for aesthetics, Audit for the built result).

## Accessibility as task completion

Behaviour mode owns whether a user can complete the task with assistive technology or constrained input. The implementation-level markup audit (roles, `aria-*`, contrast, axe output) is Audit mode's. The test at this altitude: can a keyboard-only, screen-reader, switch, or one-handed touch user finish the job, not just reach the first control?
...
- Target size is a rendered check: the audit rule owns the 44 and 24 CSS-pixel numbers (`interaction-target-size`). The behaviour decision is upstream: whether the action is on a control at all, rather than a gesture or a tiny inline glyph.

## Review scan order

In `review` mode, walk the flow once per category in `product-rules.md`, in this order: action naming and consequence (the highest-impact failures live here), state coverage, interaction and control selection, accessibility as task completion, hierarchy and structure. Stop at decision altitude: the finding is "this delete has no safeguard", not "add `onConfirm`".
...
A line-level React bug with a code patch is Audit mode's output; hand it over rather than writing the fix here.
