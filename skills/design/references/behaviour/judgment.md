...
Load in `shape`, `spec`, and `harden` modes, and for any material product or flow decision. Decide what should exist before how it looks: visual execution belongs to Build mode, the decision belongs here.
...
A gesture that replaces a control (swipe-to-delete, drag-to-reorder, hold-to-confirm) is a capability decision, settled here before Motion mode builds its physics.
