...
Stable rule IDs cited across every mode. Each finding or decision in Behaviour mode names a rule ID so it is traceable, dedupable against an Audit mode finding, and verifiable. This file owns every `rule/` ID; `ghostwriter` writes copy against the nine copy IDs. Lint rules, reviews, and exemplars reference the same slug.
...
| Enforcement | `lint` (deterministic, see `lint-patterns.md`), `judgment` (Behaviour mode), or `copy` (wording written by `ghostwriter` against these IDs) |
...
Visual-token integrity (design-system overrides, raw shadows, off-grid spacing, modal scroll structure) is a rendered or lint concern owned by Audit mode and the project's visual lint. Behaviour mode decides whether a modal should exist; whether its body scrolls correctly is Audit mode's check.
...
Behaviour mode owns these IDs; `ghostwriter` writes the strings against them, and the product decision stays here.
...
- Source: `lint-patterns.md`; `judgment.md` > Control selection; GOV.UK select and radios; NN/g dropdowns.
...
- Source: `judgment.md` > Semantics; Audit mode (`nav-semantic-links`) for the rendered check.
...
- Source: `lint-patterns.md`; `reachable-states.md` > Overlays; Material dialogs.
- Enforcement: lint plus judgment.
...
- Source: `judgment.md` > Surface persistence; NN/g modal dialogs.
- Enforcement: judgment.
...
- Source: `judgment.md` > Gestures; WCAG 2.5.1, 2.5.2, 2.5.7; Motion mode builds the gesture once the alternative is settled.
- Enforcement: judgment.
...
- Source: `judgment.md` > Smallest coherent intervention; NN/g slips (constraints, defaults, forgiving formatting).
- Enforcement: judgment.
...
- Source: `naming-and-consequence.md`; Apple HIG alerts; Polaris and Material content guidelines.
...
- Source: `naming-and-consequence.md` > Object, scope, consequence; NN/g confirmation dialogs.
- Enforcement: judgment.
...
- Source: `naming-and-consequence.md` > Reversibility decides the pattern; WCAG 3.3.4; GOV.UK check answers.
- Enforcement: judgment.
...
- Source: `reachable-states.md` > Destructive state; NN/g confirmation dialogs; NN/g heuristic 3 (user control and freedom).
- Enforcement: judgment.
...
- Source: `reachable-states.md` > Destructive state; Material snackbar; WCAG 2.2.1.
- Enforcement: judgment.
...
- Source: `reachable-states.md` > Validation and error; Audit mode (`forms-lost-data-on-error`) for the React-level check; GOV.UK error message.
- Enforcement: judgment.
...
- Source: `reachable-states.md` (the full enumeration); Hurff UI stack.
- Enforcement: judgment.
...
- Source: `reachable-states.md` > Empty state; NN/g empty states.
- Enforcement: copy plus judgment.
...
- Source: `reachable-states.md` > Validation and error; GOV.UK error message; NN/g error-message guidelines.
- Enforcement: copy plus judgment.
...
- Source: `reachable-states.md` > Loading state; `rule/loading-state-specific`.
- Enforcement: judgment.
...
- Source: `reachable-states.md` > Loading state; NN/g progress indicators.
- Enforcement: copy.
...
- Source: `reachable-states.md` > Expired and offline; WCAG 2.2.1.
- Enforcement: judgment.
...
- Source: `lint-patterns.md`; `interface-quality.md` > Accessibility as task completion; Audit mode (`a11y-icon-controls-labeled`).
- Enforcement: lint plus judgment.
...
- Source: `interface-quality.md` > Accessibility as task completion; WCAG 2.4.11; rendered checks to Audit mode (`interaction-keyboard-operable`, `focus-*`).
- Enforcement: judgment.
...
- Source: `lint-patterns.md`; `interface-quality.md`; Audit mode (`interaction-focus-visible`).
- Enforcement: lint plus judgment.
...
- Source: `reachable-states.md` > Validation and error; WCAG 3.3.7.
- Enforcement: judgment.
...
- Source: `interface-quality.md` > Accessibility as task completion; WCAG 3.3.8; Audit mode (`forms-dont-block-paste-ime`) for the built check.
- Enforcement: judgment.
...
- Source: `judgment.md` > Hierarchy and structure.
- Enforcement: judgment.
...
- Source: `judgment.md` > Hierarchy and structure; visual execution to Build mode.
- Enforcement: judgment.
...
- Source: `judgment.md` > Hierarchy and structure; permission wording to `rule/permission-benefit-first`; Apple HIG privacy.
- Enforcement: judgment.
