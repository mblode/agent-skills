...
```bash
node <skill>/scripts/verify/probe.mjs target-size --url <url>   # 360x800 touch by default
```

## What the script does

Runs at a touch viewport (360x800 with touch emulation) by default. The 44px floor is a touch threshold; measuring it under a fine pointer reports desktop-dense UI as broken. It enumerates visible interactive elements, skips `pointer-events: none` and off-canvas boxes, hit-tests a 5x5 grid inside a 44px box centred on each, and returns `candidates` (under 44, not expanded) and `expanded` (under 44, hit area expanded). Confirm ancestor event delegation separately; a hit on a generic parent does not prove it activates the child.
