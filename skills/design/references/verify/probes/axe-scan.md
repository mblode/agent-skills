...
```bash
node <skill>/scripts/verify/probe.mjs axe-scan --url <url> --theme <light|dark> --wait-for <selector>
```

## What the script does

Uses `@axe-core/playwright` when the repo has it, else injects `axe-core`'s `axe.min.js`; the results object is identical. Tags: WCAG 2.0, 2.1, and 2.2 A and AA. Third-party payment, video, and consent frames are excluded and the report says so.

Pass `--wait-for` so the page settles first: a scan fired before hydration reports missing labels on controls that are about to receive them. Wait for the route's primary content selector, not a fixed timeout.

## Mapping violations to rule ids

Emit each violation under the audit rule it corresponds to, so the finding joins the existing corpus rather than arriving as a parallel report:
...
- **Focus order and restoration.** axe checks landmarks and tab indexes, never where focus went after an action. That is the `focus-walk` probe.
