...
```bash
node <skill>/scripts/verify/probe.mjs console-network --url <url>
```

## What the script does

Attaches the listeners before the first navigation, waits for the primary content, and scrolls to the bottom to trigger lazy work. It also records image and frame requests issued before any scroll (`perf-lazy-load-offscreen`). Clicking the primary action is yours when it is non-destructive: Errors thrown on interaction outnumber errors thrown on load.

## Signatures worth naming

| Signature | What it means |
|---|---|
| `Hydration failed`, `Text content does not match server-rendered HTML` | SSR and client render diverged. On the primary route this is a ship blocker in the audit's tiering, and it is one of the few defects with no static tell |
...
Emit under `runtime:<signature>` where no audit rule owns the class, because these are browser findings and the corpus should not be made to look as though it predicted them. Two exceptions map cleanly:

| Observation | Rule id |
|---|---|
| A failed request whose failure renders no user-visible state | `states-no-error-state`, confirmed by the `failure-injection` probe |
