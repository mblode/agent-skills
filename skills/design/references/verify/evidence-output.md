...
The delta Verify mode adds to the Audit finding document, and nothing else. That schema, in `references/audit/output-adapters.md`, stays the single owner of the finding object, the three counts, the tiers, and the verdict. Two files describing one JSON shape is how the two drift apart, so what follows adds fields and redefines none.
...
|---|---|---|
| `probe` | always | the probe name, as passed to `probe.mjs` |
...
| `reruns` | always | how many times the probe ran. A timing-sensitive fail (`layout-shift`, `web-vitals`, an out-of-order race) is at least 2; a deterministic measurement needs one |
...
Keep the before artifact. Writing the after capture over the before is how a report loses its own argument. A re-run that still reproduces is recorded as `clearedBy.result: "reproduced"` on an `applied` finding, which is the evidence the audit's revert-on-failed-fix rule runs on.
...
Added to the audit's existing codes. `scripts/audit/check-report.mjs` enforces every one except `unknown-as-pass`, which needs the list of findings that were handed over:
...
"single-run-fail"          // a reproduced timing-sensitive finding with reruns < 2
