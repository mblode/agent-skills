...
- Body, paragraph, and general page content stays `text-base` (16px) at every breakpoint, per Typography in `foundations.md`, which owns body size. Desktop reading distance is longer than mobile, so stepping body copy down to `text-sm` at `sm:` makes it harder to read, not denser
...
- Small/icon buttons need their touch target expanded on coarse pointers: Buttons in `components.md` owns the size and the expander markup
