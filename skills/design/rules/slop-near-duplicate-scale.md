...
**Skip entirely:** `rem`, `%`, `vh`, `vw`, `em`, `ch`, and any value containing a function (`calc(`, `min(`, `max(`, `clamp(`, `round(`). A `rem` font size is the form `references/visual/tailwind.md` explicitly asks for, and a percentage or viewport unit is relative to something the scale cannot know about.
