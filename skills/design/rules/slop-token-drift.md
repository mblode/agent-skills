...
- **`ch` measures.** `max-w-[65ch]`, `max-w-[35ch]` and friends are mandated by `references/visual/sections.md` (Heading Groups) and `references/visual/sections.md` (Prose Content). A theme cannot express a measure in characters.
- **Anything containing `max(`, `min(`, `clamp(`, `calc(`, or `round(`.** `size-[max(100%,3rem)]` is the touch-target expander mandated by `references/visual/components.md` (Buttons); `rounded-[calc(var(--radius)-var(--padding))]` is the nested-radius formula. A function is reasoning, not a magic number.
