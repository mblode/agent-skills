...
Verify mode runs this as its `layout-shift` probe, holding the data response open rather than throttling the whole network so the shell is not slowed with it. Dispatch to it when an app is running; with no browser the greps below produce candidates and the rule is `unknown`, never a fail.
