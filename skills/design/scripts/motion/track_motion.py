...
Run:
    python3 scripts/motion/track_motion.py <framedir> [--out metrics.json]
...
These are PROXIES from pixels, not ground truth. Read references/motion/measurement-guide.md
...
def extraction_fps(framedir):
    """The fps extract_frames.py sampled at, or None for frames from elsewhere."""
    try:
        with open(os.path.join(framedir, "extract.json")) as f:
...
def main():
    ...
    with open(out, "w") as f:
        json.dump({"fps": extraction_fps(args.framedir), "frame_count": len(timeline),
                   "tracked_frames": len(present), "timeline": timeline}, f, indent=2)
