...
Run:
    python3 scripts/motion/fit_curves.py <metrics.json> [--fps N] [--property NAME]
...
overshoots will beat a bezier (and vice versa). Read references/motion/curve-fitting.md
...
def main():
    ...
    p.add_argument("--fps", type=int, default=None,
                   help=f"fps used during extraction (default: the fps recorded in metrics.json, else {DEFAULT_FPS})")
    ...
    # Fitting at a different rate than extraction scales every duration and
    # stiffness, so the recorded extraction fps wins unless overridden.
    fps = args.fps or data.get("fps")
    if fps is None:
        fps = DEFAULT_FPS
        print(f"warning: no fps recorded in {args.metrics}; assuming {DEFAULT_FPS}. Pass --fps to match extraction.",
              file=sys.stderr)
    elif args.fps and data.get("fps") and args.fps != data["fps"]:
        print(f"warning: --fps {args.fps} differs from the extraction fps {data['fps']} recorded in metrics.json.",
              file=sys.stderr)

    frames = np.array([r["frame"] for r in tl], float)
    t = (frames - frames[0]) / fps  # seconds from first tracked frame
    ...
    result = {"fps": fps, "duration_ms": duration_ms, "tracked_frames": len(tl), "properties": {}}
