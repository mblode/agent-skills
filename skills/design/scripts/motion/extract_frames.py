...
Run:
    python3 scripts/motion/extract_frames.py <video> <outdir> [--fps N] [--cols C]
...
Produces:
    ...
    <outdir>/extract.json            the sampling fps, which track_motion.py carries into
                                     metrics.json so fit_curves.py cannot fit at the wrong rate
...
def main():
    ...
    with open(os.path.join(args.outdir, "extract.json"), "w") as f:
        json.dump({"fps": args.fps, "start": args.start, "duration": args.duration, "frames": len(frames)}, f, indent=2)
