...
**Default tier:** mostly fix-this-sprint
**Rules:** 6
**Description:** Patterns that work on desktop but fail on touch: hover-only affordances, hover that sticks after tap, tap highlight with no press, pull-to-refresh on app shells, missing viewport meta, `100vh` on mobile, no safe-area insets, and a status bar that does not match the header. Lighthouse catches some tap-target failures; these rules add the affordance and chrome patterns.
...
Total: 92 rules across 19 categories.
