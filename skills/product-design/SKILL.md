...
name: product-design
description: >-
  Decides what an interface should do, before and apart from how it is built:
  the right interaction and control, actions named by object, scope, and
  consequence, coverage of every reachable state (loading, empty, error,
  permission, partial, destructive, offline), resilience, and accessibility as
  task completion. Works from a brief, spec, mockup, intent, or existing UI;
  cites stable rule IDs; defers to the project's design system and AGENTS.md.
  Routes by mode: shape, implement, review, copy, harden. Use when asked "is
  this the right interaction", "design the flow for", "what should this button
  do", "name this action", "which states should this have", "review this UX
  decision", "make this resilient", or "what breaks here". Not for building or
  styling UI (use ui-design), auditing a built or rendered result or React/Next
  code (use ui-audit), or copy wording and AI-ism removal (use copywriting).
---
...
- **IS NOT:**
  - building or styling UI in code, visual direction, palettes, type: use `ui-design`.
  - auditing the built result (rendered quality, a11y markup, keyboard, layout resilience, performance, type surface, and React/Next code-level UX with a ship verdict): use `ui-audit`.
...
4. Sibling-skill ownership. Route, do not duplicate:
   ...
   - the built result: rendered quality, a11y-markup audit, and React or Next behavior review with a ship verdict: `ui-audit`
...
For each finding: location (file and line, or rendered location and viewport), verification status, the rule ID, the user consequence, and the smallest concrete fix with the skill that owns implementing it. Keep findings at the decision altitude; a line-level code or framework fix is `ui-audit`'s output, not this skill's. Full rubric in `references/interface-quality.md`.

## Linters vs agent guidance

Deterministic, structural, single-file checks belong in a linter; judgment that needs product context stays in this skill. Some of the standards above have a deterministic slice a project's own linter can enforce: control selection by option count, nested modals, and missing accessible names. Encode those as lint rules in the consuming project, wired to that project's components; keep the judgment here. Pure visual-token checks (raw shadows, off-grid spacing, design-system overrides, modal scroll structure) are not this skill's concern: they belong to `ui-audit` and the project's visual lint.

See `references/lint-patterns.md` for the "lint rule vs agent guidance" decision tree and example rule shapes to adapt. Counting 2-3 static options is mechanical, so it belongs in a lint rule; naming the right object and consequence needs product context, so it stays here.

## Evals and the review loop

Keep this skill calibrated. Evals test whether the guidance still produces the right edits on unseen interfaces; a review loop gathers evidence and prepares guideline updates for human review. Both are recommended practice, not runtime behavior. See `references/evals.md` for the fixture shape, the judge rubric, and the collector, judge, and human-review loop.
