...
- **IS NOT:** authoring SKILL.md files (use `agent-skills-creator`), project docs or READMEs (use the external `ghostwriter` skill where installed), or mining session history (use the external `cadence-advise` skill where installed; this skill audits the file as-is).
...
- External `ghostwriter` where installed: human-facing documentation; AGENTS.md content that belongs in docs should move there.
