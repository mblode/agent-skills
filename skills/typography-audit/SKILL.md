...
- **IS NOT:** a broad UI quality review (accessibility, forms, navigation — use `ui-audit`), or a redesign that picks new typefaces and scales (use `ui-design`).
...
Category map and impact rationale: `rules/_sections.md`. Each rule file contains why the rule matters, an incorrect example, and a correct example. Rule frontmatter carries the rule's own `impact` — report findings with the rule's impact, which may differ from its category (e.g., `font-rendering` is MEDIUM inside the CRITICAL `font-` category).
...
- [HIGH] `size-line-height` (file.css:18): `line-height: 20px` — fixed value breaks at larger font sizes.
