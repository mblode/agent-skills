...
Defines the 10 rule categories: ordering (= audit priority), category impact, filename prefix, and rule count. Rule files are named `<prefix>-<slug>.md`. A rule's own frontmatter `impact` may differ from its category impact — report findings with the rule-level value.
