...
8. **Accessibility.** Hover animations gated behind `@media (hover: hover) and (pointer: fine)`. Where the project already ships `prefers-reduced-motion` handling, new motion follows suit (gentler, not zero: keep opacity/color, drop movement); its absence is not a finding in projects that don't handle it. Flag on sight: ungated `:hover` motion.
...
9. **Accessibility and cohesion**: add hover gating (and keep reduced-motion handling consistent where the project already has it); tune to match the component's personality.
...
6. **Accessibility**: pointer/hover gating (plus reduced-motion consistency, only where the project supports it).
