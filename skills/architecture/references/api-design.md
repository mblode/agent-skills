...
Environment validation runs when the entrypoint starts, not when a module is imported. A config singleton parsed at import time throws inside any test or tool that imports the module without the full environment, and a cached green run hides it until a fresh run.
...
The request context carries identity; it does not carry the database transaction implicitly unless the data layer reads it from there too. Where a DAO takes a transaction client, it uses that client for every query, and the type signature accepts only the transaction client so the global client cannot be substituted.
