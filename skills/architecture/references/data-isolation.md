...
Tenancy workflow step 3, on any platform. Pick a model per plan tier, then implement the enforcement so a missing `WHERE tenant_id` cannot leak.

## Contents

- Choosing a model
- Shared schema with tenant_id and RLS (default)
- RLS implementation notes (Prisma, Drizzle, Supabase, Neon)
- Schema-per-tenant
- Database-per-tenant
- Evidence
- Sources
...
- **Prisma**: run each request's queries in an interactive transaction, ``prisma.$transaction(async (tx) => { await tx.$executeRaw`select set_config('app.tenant_id', ${id}, true)`; ... })``, and pass `tx` to every DAO call. A DAO that falls back to the module-level `prisma` client runs on another connection with no tenant set, which under forced RLS returns nothing and under an owner or `BYPASSRLS` connection returns everything. Keep two datasource URLs: the migration role for `prisma migrate`, the app role for the running app.
...
- **Neon**: one project per tenant via the API; idle compute scales to zero, so idle tenants cost little. Per-tenant point-in-time restore and region choice come with the model. Included project counts and scale-to-zero timing vary by plan: look them up live.
- **D1**: one database per tenant, within the per-account database cap and per-database size limit (look both up live); bind the tenant's database in its user Worker or open it by id from the dispatch Worker.
