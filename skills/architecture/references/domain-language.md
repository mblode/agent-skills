...
```markdown
# 0007 Enum values are UPPER_SNAKE, response fields stay camelCase

Choice: enum values are UPPER_SNAKE; response fields stay camelCase.
Reason: our first two API consumers hand-wrote switch statements over enum
values and both got bitten by casing drift. UPPER_SNAKE makes enum values
visually distinct from fields at the call site.
Flip condition: a generated client that owns every enum comparison, so no
consumer hand-writes a switch.
```

Three lines are the whole template: the choice, the reason, and the flip condition (the observation that would change the decision). The flip condition is the part most often missing and the one that makes a record checkable: without it, a later reader cannot tell whether the decision still holds or has quietly expired.

Status, Considered Options, and Consequences are optional and most records will not need them. Add Considered Options only when a rejected alternative is likely to be proposed again.
...
- A decision record with no flip condition. Nobody can tell when it stopped being true.
