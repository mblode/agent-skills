...
Flows that call external systems, consume webhooks, use queues or an outbox, retry, need an audit trail, or move money. Three principles, each enforced by a test, barrier, or constraint:
...
- **Claim, then enqueue, in one transaction.** The relay claims rows with `SELECT ... FOR UPDATE SKIP LOCKED` and either enqueues and marks them sent before commit (queue in the same database, as with pg-boss) or sets a lease column that a second relay respects. A lock held only for the `SELECT`, with the enqueue after commit, lets two relays enqueue the same row.
- **Unknown event types fail loudly.** A row whose type has no registered handler goes to a dead-letter state with an alert, never stays `pending` forever.
- **Queues exist before anything registers against them.** Create them in a migration or an idempotent startup step that runs before workers register; a fresh database otherwise fails on first send or silently drops.
- Test the relay with two concurrent relays against one outbox and assert each row is enqueued exactly once.
