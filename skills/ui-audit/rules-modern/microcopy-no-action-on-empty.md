...
- Skip read-only / archival surfaces where no action is possible by design (e.g. a closed-period transaction list). Mark with `// ui-audit-ignore:microcopy-no-action-on-empty, read-only archive`.
...
```tsx
{/* ui-audit-ignore:microcopy-no-action-on-empty, read-only archive of closed periods */}
