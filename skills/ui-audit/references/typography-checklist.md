...
- [ ] Line length 45-75 characters (66 ideal), adjusted per breakpoint — `max-width` in `ch` units.
- [ ] Line height ~1.45-1.6, unitless; a bit more for large-x-height sans faces; tighter for large headlines.
- [ ] Fluid sizes via `clamp()`; no near-equal sizes in the scale — same or clearly different.
- [ ] Widows/orphans managed: `text-wrap: balance` on headings or non-breaking spaces in headlines/nav.

## Weights, styles, and OpenType

- [ ] Real regular/italic/bold/bold-italic loaded via `@font-face` mapped to one family — no faux bold or faux italic.
...
- [ ] Heading colour distinct from link colour; large headings may lighten weight/colour for balance — prefer darkened brand hues over flat grey.
