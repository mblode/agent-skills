...
Be liberal in what you accept, conservative in what you send (Jon Postel, RFC 760, 1980). On input, tolerate the messy ways real people type: uppercase emails, phones with spaces or dashes, dates in any reasonable format, leading/trailing whitespace. Normalize on the way in.

On output, be precise: error messages specific, actionable, unambiguous. The asymmetry is the point: forgive input, but tell the user clearly what happened and what to do next.
...
3. Find error messages near inputs (`role="alert"`, `aria-invalid`, `<FormError>`). Bare strings (`"invalid"`, `"error"`, `"please try again"`) or error codes alone are vague.
