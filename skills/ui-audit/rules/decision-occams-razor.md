...
Among solutions that perform equally well, prefer the one with the fewest moving parts. In UI, "moving parts" means elements, states, copy, options, dependencies, exceptions. The cheapest complexity to remove is the complexity you never add. Rubric-based rule: score how well the surface keeps the primary task foregrounded against the closest anchor.

Density is not clutter. A high-density dashboard can still score 5 if every element is task-relevant. Penalize only when elements are decorative, duplicated, or compete with the primary task.
...
| Score | Anchor |
|---|---|
| 5 | Every visible element earns its place: primary task foregrounded, no decorative widgets, no controls duplicated across regions. |
| 4 | Mostly clean; one or two decorative elements (illustration, "fun" empty-state graphic) that don't compete with the task. |
...
| 1 | More decorative elements than functional ones; task discovery requires hunting. |
...
- Consolidate duplicated controls (one save button, one nav, one CTA per region); delete the rest.
...
Profile and team metadata move into a post-signup setup step, gated only when the user needs that capability.
