...
- **Commit secrets** — API keys, tokens, passwords, private keys in source (use environment variables or secret managers)
...
- **Trust client-sent IDs for authorization** — always verify ownership server-side (e.g., `where user_id = currentUser.id`)

## OWASP Quick Reference

When reviewing, check the diff against these common vulnerability classes:
...
| Insufficient Logging | Auth events not logged, no audit trail for sensitive operations |
