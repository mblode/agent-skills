...
- Code introduces a concrete security risk with direct exploit path — load `references/security-checklist.md` for the three-tier classification (Always Do / Ask First / Never Do) when the diff touches auth, input handling, external APIs, or environment configuration
