...
How to read `fit_curves.py` output, choose spring vs cubic-bezier, judge fit quality, and
handle asymmetric open/close curves.
...
- Report two curves. In code, give the enter and exit transitions different
  `duration`/easing (and different spring configs) rather than a single shared one.
