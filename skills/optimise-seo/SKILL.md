...
- Under `cacheComponents: true` (the `scaffold-nextjs` default) every route segment config is a build error: `export const revalidate` on `app/sitemap.ts`, `dynamic = 'force-static'` on `robots.txt/route.ts`, `fetchCache` anywhere. The message is `Route segment config "dynamic" is not compatible with nextConfig.cacheComponents`. Move the content fetch into a `'use cache'` helper with `cacheLife('hours')` and `cacheTag`, and `revalidateTag(tag, 'max')` from the publish webhook.
...
- A `/.well-known/<name>.json` path with a single dotted segment never reaches `next.config.ts` rewrites; Next answers with a prerendered 404 first. Two-segment paths (`/.well-known/mcp/server-card.json`) rewrite fine. For the single-segment case, rewrite in `proxy.ts` with the literal path in the matcher, and keep the handler at a non-dotted route. Dotfiles under `public/.well-known/` are dropped by the static pipeline as well.
- Route handlers that are statically prerendered (`sitemap.ts`, `robots.ts`, `llms.txt`) run at build time and never again, so an analytics capture inside one fires once per deploy. Per-request capture belongs in `proxy.ts` behind `event.waitUntil()`. And a `try/catch` in a `GET` handler catches the prerender bail-out too; set `experimental.hideLogsAfterAbort: true` or every cached handler logs a stack trace during the build.
...
| Schema versus DOM | `curl -s <url>`, then compare every JSON-LD `name`, `text`, `headline`, and breadcrumb label against the rendered HTML with scripts stripped | every claim visible on the page; exactly one `application/ld+json` block per page (count `<script` tags, not string occurrences: the RSC payload repeats the escaped string) |
