...
Every runtime dependency you add is one the user installs, audits, and ships. Avoid pulling a heavy library for one helper, and move framework-version expectations (React, the runtime, a bundler) into `peerDependencies` so a mismatch is a clear warning at install time, not a cryptic crash at runtime.
